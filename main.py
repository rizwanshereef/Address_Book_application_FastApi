...
'''Address book application where API users can create, update and delete
addresses.'''
...
logging.basicConfig(filename='./log/log_file.log', format=FORMAT,
...
# Fetching address based on the put request
    db_address = db.query(AddressesModel).filter(
        AddressesModel.sl_no == address_id).first()
    if not db_address:
...
# Calculating distance between locations in DB vs locations from Request and selecting the nearest location based on distance
    for row in addresses:
        logging.debug('Calculated distance:%s', GD(
              user_location, (row.latitude, row.longitude)).km)
        ...
        if not nearby_addresses:
                raise HTTPException(
                    status_code=404, detail='No nearby address')
